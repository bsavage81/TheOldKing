...
role_ids = [816411231233114133,816418524561670185]

Aurafall = 298995889551310848
Coastal = 305767872410419211
Combine = 277850067992313857
MRP = 587495640502763521
PBtest = 448488274562908170
...
class ButtonRoleCog(commands.Cog):
    ...
    @slash_command(guild_ids=[PBtest], description="Post the button role message")
    ...
    async def on_ready(self):
        ...
        view = discord.ui.View(timeout=None)
        # make sure to set the guild ID here to whatever server you want the buttons in
        guild = self.bot.get_guild(PBtest)
        for role_id in role_ids:
